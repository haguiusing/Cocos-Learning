    ...
    execute: function () {
      ...
      _export("Lesson01_Animation", Lesson01_Animation = (_dec = ccclass('Lesson01_Animation'), _dec(_class = class Lesson01_Animation extends Component {
        ...
        start() {
          ...
          const [x, y, z] = track.channels(); // 为 x 通道的曲线添加关键帧
          ...
          const vec3KeyFrames = [[0.4, new Vec3(1.0, 2.0, 3.0)], [0.6, new Vec3(1.0, 2.0, 3.0)], [0.8, new Vec3(1.0, 2.0, 3.0)]];
          x.curve.assignSorted(vec3KeyFrames.map(([time, vec3]) => [time, {
            value: vec3.x
          }]));
          y.curve.assignSorted(vec3KeyFrames.map(([time, vec3]) => [time, {
            value: vec3.y
          }]));
          z.curve.assignSorted(vec3KeyFrames.map(([time, vec3]) => [time, {
            value: vec3.z
          }])); // 将轨道添加到动画剪辑
        ...
        specifyTrackPath() {
          // 创建一个轨道路径
          const trackPath = new animation.TrackPath(); // 指定目标对象为当前节点的 "path/to/child" 子节点的 "MyComponent" 组件的 "myProperty" 属性的第二个数组元素
        ...
        createValueProxy() {
          // 创建一个轨道路径
          const trackPath = new animation.TrackPath(); // 指定目标对象为材质
