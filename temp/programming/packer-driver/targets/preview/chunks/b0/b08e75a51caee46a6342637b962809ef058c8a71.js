  ...
  return {
    ...
    execute: function () {
      ...
      _export("Lesson31_UITransform", Lesson31_UITransform = (_dec = ccclass('Lesson31_UITransform'), _dec(_class = class Lesson31_UITransform extends Component {
        ...
        //#region 1. 尺寸与锚点
        demoSizeAnchor() {
          var ui = this.getComponent(UITransform); // 设置内容尺寸
        ...
        demoCoordinate() {
          var ui = this.getComponent(UITransform); // 节点局部坐标 → 世界坐标

          var worldPos = ui.convertToWorldSpaceAR(new Vec3(0, 0, 0)); // 世界坐标 → 节点局部坐标

          var localPos = ui.convertToNodeSpaceAR(worldPos);
        } //#endregion
        ...
        demoBoundingBox() {
          var ui = this.getComponent(UITransform); // 自身在父节点坐标系中的包围盒

          var localBox = ui.getBoundingBox(); // 世界坐标系下包含子节点的总包围盒

          var worldBox = ui.getBoundingBoxToWorld(); // 计算世界空间 AABB

          var aabb = ui.getComputeAABB();
        } //#endregion
        ...
        demoHitTest() {
          var ui = this.getComponent(UITransform); // 判断本地 UI 坐标点是否命中当前节点（不推荐使用）
          // const hitUI = ui.isHit(new Vec2(100, 100));
          // 推荐使用新版 hitTest（屏幕坐标 → 节点局部）

          var screenPoint = new Vec2(200, 300);
          var hitNode = ui.hitTest(screenPoint);
        } //#endregion
        ...
        demoPriority() {
          var ui = this.getComponent(UITransform); // 设置同一层级下的渲染顺序（数值越大越靠前）
        ...
        dynamicAdjust() {
          var ui = this.getComponent(UITransform); // 每帧让节点尺寸逐渐增大
